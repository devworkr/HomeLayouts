...
class LayoutsPostsTypes {
    ...
    public static function init() {
        register_post_type('home_layouts', LayoutsPostsTypes::instance()->get_post_arg());
        add_action('pre_get_posts', array(LayoutsPostsTypes::instance(), 'layout_posts_order'));
        add_action('add_meta_boxes', array(LayoutsPostsTypes::instance(), 'home_layouts_metabox'));
        add_action('save_post', array(LayoutsPostsTypes::instance(), 'save_layout_metaboxes'));

        add_filter('get_terms', array(LayoutsPostsTypes::instance(), 'layout_get_object_terms'), 10, 3);
        add_filter('wp_get_object_terms', array(LayoutsPostsTypes::instance(), 'layout_get_object_terms'), 10, 3);

        /*add_action('admin_menu', array(LayoutsPostsTypes::instance(), 'layout_post_categories'));*/
    }
    ...
    public function home_layouts_metabox() {
        add_meta_box(
                'layout_style', __('Layout', 'is-layouts'), array(&$this, 'layout_meta_box_callback'), 'home_layouts'
        );
        add_meta_box(
                'category_id', __('Category', 'is-layouts'), array(&$this, 'category_id_meta_box_callback'), 'home_layouts'
        );



        add_meta_box(
                'selected_posts', __('Posts', 'is-layouts'), array(&$this, 'selected_posts_meta_box_callback'), 'home_layouts'
        );
    }
    ...
    function save_layout_metaboxes($post_id) {
        ...
        if (isset($_POST['post_type']) && 'home_layouts' == $_POST['post_type']) {
    ...
    private function get_post_arg() {
        $labels = array(
            ...
            'not_found' => __('Not home layout found. Please add some layouts.', 'is-layouts'),
            'not_found_in_trash' => __('Not found in Trash', 'is-layouts'),
        );

        $args = array(
            'label' => __('layouts', 'is-layouts'),
            'description' => __('Movie news and reviews', 'is-layouts'),
            'labels' => $labels,
    ...
    function layout_get_object_terms($terms) {
        $tags = ['category'];
